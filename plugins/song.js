...
Module({
    ...
    if (link !== null && getID.test(link[0])) {
        ...
        ffmpeg(stream)
            ...
            .on('end', async () => {
                try {
                    var audio = await addInfo('./temp/song.mp3', details.title, AUDIO_DATA.split(";")[1], "faz Engine", thumb)
                } catch {
                    return await message.client.sendMessage(message.jid, {
...
Module({
    ...
    if (message.list && message.list.startsWith("ytsl") && message.list.includes(message.client.user.id.split("@")[0].split(":")[0])) {
        ...
        var {
            info,
            thumbnail
        } = await getJson("https://faz-api.vercel.app/api/youtube/details?video_id=" + message.list.split(";")[1]);
        const buttonMessage = {
